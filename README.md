...
$$a=f(\underbrace{\overbrace{\text{sum}(\underbrace{w\circ x}_{H})}^{S} + b}_z)$$
...
$$\text{softmax}(z)\_j = \frac{e^{z\_j}}{\sum_{k=1}^{C} e^{z\_k}}$$
...
$$\hat{y_j} = \text{softmax}(z)\_j = \frac{e^{z\_j}}{\sum_{k=1}^{C} e^{z\_k}}$$
